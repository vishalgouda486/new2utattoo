...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    ...
    const elements = document.querySelectorAll(
      "a, button, .glass, .portfolio-card, .tilt-3d, .gallery-card"
    );
    elements.forEach((el) => {
              ...
              >
                <div className="relative h-[64vh] min-h-[420px] max-h-[760px] overflow-hidden sm:h-[70vh] sm:min-h-[500px]">
                  ...
                  {item.id === "portfolio-5" ? (
                    ...
                    <div
                      className="absolute inset-0 bg-cover bg-center grayscale brightness-[0.42] transition duration-700 group-hover:grayscale-0 group-hover:scale-110 group-hover:brightness-[0.96] mobile-scroll-reveal mobile-image-fx"
        ...
        <div className="columns-2 gap-3 sm:columns-2 md:columns-3 xl:columns-4 sm:gap-4">
          {galleryImages.map((img, i) => (
            <motion.div
              ...
              className="gallery-card group relative mb-3 break-inside-avoid overflow-hidden rounded-[1.7rem] border border-white/10 bg-black/40 sm:mb-4"
            >
              <img
                src={img}
                alt={`Tattoo work ${i + 1}`}
